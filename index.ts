...
// query(arr).filter(x => x < 0.5).map(x => x + 1).toArray();
...
//     fs.writeFileSync('dima.cpuprofile', result);
//     profile.delete();
//     process.exit();
//   });
// }, 2000);
...
const arr = [1, 2, 3, 4, 5, 33, 3454, 34, 44];
const i1 = iterate(arr);
const i2 = query(arr);
console.log(i1.filter(x => x < 300).toArray());
console.log(i2.filter(x => x < 300).toArray());
